...
import { MDX_ESSAY_SLUGS, WORK } from '@/lib/work'
...
const declaredSlugs = [...MDX_ESSAY_SLUGS].sort()
if (bodyKeys.join(',') !== declaredSlugs.join(',')) {
  throw new Error(
    `lib/essay-bodies.ts's BODIES keys (${bodyKeys.join(', ')}) must match lib/work.ts's MDX_ESSAY_SLUGS (${declaredSlugs.join(', ')})`,
  )
}
for (const slug of MDX_ESSAY_SLUGS) {
  if (!WORK.some((item) => item.slug === slug)) {
    throw new Error(`MDX_ESSAY_SLUGS lists "${slug}", but no WorkPiece in lib/work.ts has that slug`)
  }
}
